...
4. When the visitor names a position P:  
   ...
   - then issue:
      #turn 10    
   - then issue:
       #camera without quotes
       From the camera image, identify a famous painting on the camera image, say a description of this painting out loud.
   - After describing the painting, output:
      #say "do you have further questions about this painting?"    
   - if no, return to original orientation:  
       If in Classic and (P == "middleright" or P == "rightmost"):  
           #turn 80  
       If in Modern and (P == "leftmost" or P == "middleleft"):  
           #turn -80
...
class MuseumGuide(StateMachineProgram):
    ...
    class CmdWait(StateNode):
      def start(self, event):
          super().start(event)
          tokens = event.data.split()
          if len(tokens) < 2:
              print("No duration provided for #wait command.")
              self.post_failure()
              return
          try:
              duration_ms = int(tokens[1])
              print(f"Waiting for {duration_ms} milliseconds.")
              time.sleep(duration_ms / 1000.0)
              self.post_completion()
          except ValueError:
              print("Invalid duration for #wait command.")
              self.post_failure()
    ...
    def setup(self):
        ...
        #       dispatch =D(re.compile('#wait '))=> self.CmdWait() =CNext=> dispatch
        ...
        # Code generated by genfsm on Tue Apr 22 18:17:53 2025:
        ...
        cmdwait1 = self.CmdWait() .set_name("cmdwait1") .set_parent(self)
        ...
        datatrans14 = DataTrans(re.compile('#wait ')) .set_name("datatrans14")
        datatrans14 .add_sources(dispatch) .add_destinations(cmdwait1)
        
        cnexttrans7 = CNextTrans() .set_name("cnexttrans7")
        cnexttrans7 .add_sources(cmdwait1) .add_destinations(dispatch)
        ...
        cnexttrans9 .add_sources(pickup) .add_destinations(dispatch)
        ...
        cnexttrans11 .add_sources(say2) .add_destinations(dispatch)
        
        cnexttrans12 = CNextTrans() .set_name("cnexttrans12")
        cnexttrans12 .add_sources(goto_aruco) .add_destinations(dispatch)
        ...
        cnexttrans13 .add_sources(say3) .add_destinations(dispatch)
        ...
        cnexttrans14 .add_sources(say4) .add_destinations(dispatch)
